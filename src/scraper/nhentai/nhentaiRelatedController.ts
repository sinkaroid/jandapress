...
import { NhentaiV2Detail, NhentaiV2GallerySummary, NhentaiV2RelatedResponse, NhentaiV2Tag } from "../../interfaces/nhentai-v2";
...
async function resolveTagMap(items: NhentaiV2GallerySummary[]): Promise<Map<number, NhentaiV2Tag>> {
  ...
  for (let i = 0; i < ids.length; i += CHUNK_SIZE) {
    ...
    try {
      ...
      // Keep compatible response when tag resolver endpoint is flaky.
    }
  }
